...
export const GET = withErrorHandling(async (_req: NextRequest, context: RouteContext) => {
  ...
  // FULL-pinned: this route feeds the two-column editor, and a basic CV read
  // through it would reach `ResumePreview` as a shape it cannot render. A basic
  // CV reads through /api/basic-resume/[id] instead.
  const resume = await findOwnedResume(user.id, id, 'FULL');
  if (!resume) {
